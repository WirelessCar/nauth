...
func TestNewOperatorNatsCluster(t *testing.T) {
	t.Run("should_succeed_and_trim_values", func(t *testing.T) {
		...
		if cluster.ClusterRef.Namespace != "operator-system" {
			t.Fatalf("expected trimmed namespace, got %q", cluster.ClusterRef.Namespace)
		}
		if cluster.ClusterRef.Name != "nats-main" {
			t.Fatalf("expected trimmed name, got %q", cluster.ClusterRef.Name)
		}
		if !cluster.Optional {
