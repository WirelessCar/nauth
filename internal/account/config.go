...
func NewConfig(operatorNatsCluster *OperatorNatsCluster, operatorNamespace string, defaultNatsURL string) (*Config, error) {
	config := &Config{
		...
		DefaultNatsURL:      strings.TrimSpace(defaultNatsURL),
	}
...
func (c *Config) validate() error {
	...
	if c.OperatorNamespace != "" {
		if errs := k8sval.ValidateNamespaceName(c.OperatorNamespace, false); len(errs) > 0 {
			return fmt.Errorf("invalid operator namespace %q: %s", c.OperatorNamespace, strings.Join(errs, ", "))
		}
	}
	if c.DefaultNatsURL != "" {
...
func NewOperatorNatsCluster(clusterRef v1alpha1.NatsClusterRef, optional bool) (*OperatorNatsCluster, error) {
	cluster := &OperatorNatsCluster{
		ClusterRef: v1alpha1.NatsClusterRef{
			Namespace: strings.TrimSpace(clusterRef.Namespace),
			Name:      strings.TrimSpace(clusterRef.Name),
		},
		Optional: optional,
	}
...
func (c *OperatorNatsCluster) validate() error {
	namespace := strings.TrimSpace(c.ClusterRef.Namespace)
	name := strings.TrimSpace(c.ClusterRef.Name)
