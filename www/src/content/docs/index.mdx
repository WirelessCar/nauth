---
title: NAuth
description: Simplify NATS operator and account management for Kubernetes
template: splash
editUrl: false
hero:
  tagline: Simplify NATS operator and account management for Kubernetes
  image:
    file: ../../../public/nauth.svg
  actions:
    - text: Get Started
      link: ./guides/getting-started
      icon: right-arrow
      variant: primary
    - text: View on GitHub
      link: https://github.com/wirelesscar/nauth
      icon: github
---

import { CardGrid, LinkCard } from '@astrojs/starlight/components';
import IconKubernetes from "~icons/devicon/kubernetes";
import FluentColorShieldCheckmark48 from '~icons/fluent-color/shield-checkmark-48';
import Card from "../../components/Card.astro";

## Why NAuth?

<CardGrid stagger>
  <Card title="Kubernetes Native">
    <IconKubernetes slot="icon" class="icon" size="1.333em" /> 
    Built specifically for Kubernetes with Custom Resource Definitions (CRDs) for seamless integration.
  </Card>
  
  <Card title="Secure by Default">
    <FluentColorShieldCheckmark48 slot="icon" class="icon" size="1.333em" />
    Keeps the operator root seed out of the controller and uses Kubernetes-native Secret management for credentials.
  </Card>
  
  <Card title="Easy to Use" icon="approve-check">
    Simple YAML configurations that abstract away the complexity of NATS operator management.
  </Card>
</CardGrid>

## Documentation

<CardGrid>
  <LinkCard
    title="Getting Started"
    icon="rocket"
    href="./guides/getting-started/"
    description="Install NAuth, configure NATS credentials, and manage Accounts and Users."
  />
  <LinkCard
    title="Observe Existing Accounts"
    icon="document"
    href="./guides/observe-existing-accounts/"
    description="Read existing NATS account claims without taking ownership before migrating them to NAuth."
  />
  <LinkCard
    title="Observability"
    icon="chart"
    href="./guides/observability/"
    description="Scrape controller-runtime metrics and monitor reconciliation behavior."
  />
  <LinkCard 
    title="API Reference" 
    icon="document"
    href="./crds/"
    description="Complete API documentation for all Custom Resource Definitions."
  />
  <LinkCard
    title="Examples"
    icon="open-book"
    href="https://github.com/WirelessCar/nauth/tree/main/examples"
    description="Runnable Account, User, import/export, and cluster reference scenarios."
  />
</CardGrid>
