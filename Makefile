...
manifests: controller-gen ## Generate WebhookConfiguration, ClusterRole and CustomResourceDefinition objects.
	$(CONTROLLER_GEN) crd webhook paths="./..." output:crd:artifacts:config=charts/nauth/resources/crds
	$(CONTROLLER_GEN) crd webhook paths="./..." output:crd:artifacts:config=charts/nauth-crds/crds
...
verify-go-version-sync: ## Verify that Go versions are aligned across go.mod, mise.toml and Dockerfile.
...
.PHONY: build
build: verify-go-version-sync manifests generate fmt vet ## Build manager binary.
	go build -o bin/manager cmd/main.go

.PHONY: run
run: verify-go-version-sync manifests generate fmt vet ## Run a controller from your host.
	go run ./cmd/main.go
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name nauth-builder
	$(CONTAINER_TOOL) buildx use nauth-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm nauth-builder
	rm Dockerfile.cross
...
GOLANGCI_LINT_VERSION ?= v$(shell awk '$$1 == "golangci-lint" { print $$2; exit }' .tool-versions)
